...
export default function Enquiry()
{
	...
	const validate = (values) => {
		...
		if (! values.name.match('[A-Za-z]+') )
			errors.name = "name shud contain only alphabtes";
	...
	const onSubmit = (values, {resetForm}) => {
		...
		let urladd = "https://enquiryapp-g3bbp0edj-giteshlokhande.vercel.app/save";
	...
	<form onSubmit = {formik.handleSubmit}>
		<input type="text" name="name" placeholder="enter ur name" onChange={formik.handleChange} onBlur={formik.handleBlur} value={formik.values.name} />
		...
		<textarea name="query" placeholder="enter ur query" rows={5} cols={30} onChange={formik.handleChange} onBlur={formik.handleBlur}
